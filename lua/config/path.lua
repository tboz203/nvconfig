--- config.path
---
--- Goal: Define a class similar to Python's `Path`.
...
function Path.__call(path_input)
  -- If we already have a Path, just return it
  ...
  if type(path_input) == "table" then
    sep = path_input.sep or path_input.sep
    path_input.sep = nil
  end
...
function Path:absolute()
  if self:is_absolute() then
    ...
    return normalize_path(self._absolute or table.concat({ self._cwd, self.filename }, self._sep), self._cwd)
  end
end
...
function Path:expand()
  ...
  if string.find(self.filename, "~") then
    expanded = string.gsub(self.filename, "^~", M.home)
...
function Path:normalize(cwd)
  ...
  local home = M.home
  if string.sub(M.home, -1) ~= M.sep then
    home = home .. M.sep
  end
  local start, finish = string.find(self.filename, home, 1, true)
  if start == 1 then
    self.filename = "~" .. M.sep .. string.sub(self.filename, (finish + 1), -1)
  end

  return normalize_path(clean(self.filename), self._cwd)
end
...
local _get_parent = (function()
  ...
  return function(abs_path)
    ...
    if parent ~= nil and not parent:find(M.sep) then
      return parent .. M.sep
    end
    return parent
  end
...
function Path:parent()
  return Path:new(_get_parent(self:absolute()) or M.root(self:absolute()))
end
...
function Path:parents()
  ...
  until not cur
  table.insert(results, M.root(self:absolute()))
  return results
end
